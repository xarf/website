...
| `reporter` | object | 🟠 Mandatory | Organization that identified the abuse (requires: org, contact, domain) |
| `sender` | object | 🟠 Mandatory | Organization transmitting the report (requires: org, contact, domain) |
...
Information about the organization that **identified or complained about the abuse**. This is the actual complainant (e.g., brand owner, ISP customer, security researcher).
...
Name of the organization that identified the abuse.
...
Contact email address of the complaining organization.

**Example Values:**
```json
"contact": "abuse@example.com"
"contact": "reports@security-org.net"
"contact": "phishing@bank.com"
```

**Validation Rules:**
- Must be valid email format (RFC 5322)
- Should be monitored inbox for the actual complainant
- Use real contact, not noreply
...
Domain name of the reporting organization.

**Example Values:**
```json
"domain": "example.com"
"domain": "security-org.net"
"domain": "bank.com"
```

**Validation Rules:**
- Must be valid domain name format
- Should match organization's primary domain
- Used for organizational identification and verification
...
Information about the organization that **is transmitting this report**. This is the infrastructure/platform sending the report (e.g., Abusix, reporting platform, mail system).

**In most cases, sender and reporter are the same organization.** Use different values only when using a reporting infrastructure provider.
...
Name of the organization transmitting the report.

**Example Values:**
```json
"org": "Abusix"
"org": "Example Security Platform"
"org": "Anti-Abuse Service"
```
...
Contact email address for the sending organization.

**Example Values:**
```json
"contact": "reports@abusix.com"
"contact": "noreply@reporting-platform.com"
"contact": "abuse@example.com"
```

**Validation Rules:**
- Must be valid email format (RFC 5322)
- Should be contact for sender infrastructure issues
- Can be noreply address for automated platforms
...
Domain name of the sending organization.

**Example Values:**
```json
"domain": "abusix.com"
"domain": "reporting-platform.com"
"domain": "example.com"
```
...
### Reporter vs Sender: When to Use Different Values

**Same Organization (90% of cases):**
```json
"reporter": {
...
**Different Organizations (infrastructure providers):**

**Use Case 1: ISP using reporting platform**
```json
"reporter": {
  "org": "Swisscom Abuse Desk",
...
**Use Case 2: Brand protection service**
```json
"reporter": {
  "org": "Major Bank Corp",
  "contact": "phishing@majorbank.com",
  "domain": "majorbank.com"
},
"sender": {
  "org": "BrandShield Protection",
  "contact": "reports@brandshield.com",
  "domain": "brandshield.com"
}
```

**Use Case 3: National CERT**
```json
"reporter": {
  "org": "University IT Security",
  "contact": "security@university.edu",
  "domain": "university.edu"
},
"sender": {
  "org": "National CERT",
  "contact": "reports@cert.gov",
  "domain": "cert.gov"
}
```

**When to Use Different Reporter vs Sender:**
- ISP abuse desks using third-party infrastructure (ISP = reporter, platform = sender)
- Brand protection companies (brand = reporter, protection service = sender)
- Anti-abuse services forwarding customer reports (customer = reporter, service = sender)
- National CERTs reporting for members (member = reporter, CERT = sender)
- MSSPs sending on behalf of clients (client = reporter, MSSP = sender)
...
- ✅ `reporter.domain` - Organization domain
- ✅ `sender.org` - Sending organization name
- ✅ `sender.contact` - Sending organization contact
- ✅ `sender.domain` - Sending organization domain
